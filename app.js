...
const tabBtnProducao =
  document.getElementById("tabBtnProducao");
...
const newProductTypeCustomGroup =
  document.getElementById(
    "newProductTypeCustomGroup"
  );

const newProductTypeCustom =
  document.getElementById(
    "newProductTypeCustom"
  );
...
const pedidoAdminCreate =
  document.getElementById(
    "pedidoAdminCreate"
  );

const createPedidoForm =
  document.getElementById(
    "createPedidoForm"
  );
...
const pedidoDataSaida =
  document.getElementById(
    "pedidoDataSaida"
  );

const pedidoObservacao =
  document.getElementById(
    "pedidoObservacao"
  );

const pedidoProductPreview =
  document.getElementById(
    "pedidoProductPreview"
  );
...
const pedidoAddToListBtn =
  document.getElementById(
    "pedidoAddToListBtn"
  );

const pedidoDraftList =
  document.getElementById(
    "pedidoDraftList"
  );
...
async function loadUserProfile() {
  ...
  if (
    ...
    tabBtnProducao?.classList.remove(
      "hidden"
    );
...
clearProductionArea.classList.remove(
  ...
  pedidoAdminCreate?.classList.remove(
    "hidden"
  );
    ...
    // Expedição: só saída, ver pedidos, ver/alterar estoque e ver
    // histórico. Não produz e não apaga/exclui nada.
    userRole.textContent =
      "🚚 Expedição";
    ...
    // Expedição PODE alterar estoque manualmente.
    editStockToggleBtn?.classList.remove(
      "hidden"
    );

    pedidoAdminCreate?.classList.add(
      "hidden"
    );
...
// ======================================================
// VERIFICAR CARGO — EXPEDIÇÃO
// ======================================================

function isExpedicaoRole(
  cargo
) {
...
function updatePedidoFields() {
  ...
  if (
    ...
    pedidoGrams.innerHTML =
      `<option value="">Selecione...</option>`;

    pedidoWeight.innerHTML =
      `<option value="">Selecione...</option>`;

  } else {

    pedidoFlavor.innerHTML =
      `<option value="">Selecione...</option>`;
...
// ======================================================
// ENCONTRAR PRODUTO DO PEDIDO
// ======================================================

function getSelectedPedidoProduct() {
...
function updatePedidoSelectionPreview() {

  const product =
    getSelectedPedidoProduct();

  if (!product) {

    pedidoProductPreview.textContent =
      "Selecione o produto";
    ...
    filteredProducts
      .map(
        item => {
          ...
          // Busca no catálogo carregado (products) o is_revenda /
          // revendedor, pois listar_estoque_produtos pode não
          // devolver essas colunas.
          const catalogProduct =
            products.find(
              p =>
                Number(p.id) ===
    ...
    pedidos
      .map(
        pedido => {
          ...
          const alvo =
            Number(
              pedido.quantidade_alvo ||
              0
            );

          const atual =
            Number(
              pedido.quantidade_atual ||
              0
            );
                ...
                <div class="pedido-actions">
                  ...
                  >
                    <input
                      ...
                      min="1"
                      step="1"
                      placeholder="Qtd. depositada agora"
                      required
                    >
                    <button type="submit">
                      ➕ Registrar
                  ...
                  <button
                    type="button"
                    class="secondary pedido-mark-ready"
                    data-pedido-id="${pedido.id}"
                  >
                    ✅ Marcar como pronto
                  </button>
                  ...
                  <button
                    type="button"
                    class="secondary danger pedido-cancel"
                    data-pedido-id="${pedido.id}"
                  >
                    🗑️ Cancelar pedido
                  </button>
...
pedidosList?.addEventListener(
  "submit",
  async event => {

    const form =
      event.target.closest(
        ".pedido-progress-form"
      );
    ...
    if (result.error) {
      ...
      console.error(
        "Erro ao registrar progresso do pedido:",
        result.error
      );
...
pedidosList?.addEventListener(
  ...
  async event => {
    ...
    if (markBtn) {
      ...
      const pedidoId =
        Number(
          markBtn.dataset.pedidoId
        );

      if (
        !confirm(
          "Marcar este pedido como pronto? Isso não altera o estoque, apenas o status do pedido."
        )
      ) {
        return;
      }
    ...
    if (cancelBtn) {
      ...
      const pedidoId =
        Number(
          cancelBtn.dataset.pedidoId
        );

      if (
        !confirm(
          "Cancelar este pedido? Ele deixará de aparecer na lista."
        )
      ) {
        return;
      }
...
pedidoType?.addEventListener(
  "change",
  updatePedidoFields
);

pedidoFlavor?.addEventListener(
  "change",
  updatePedidoSelectionPreview
);

pedidoGrams?.addEventListener(
  "change",
  () => {
...
// ======================================================
// LISTA DE RASCUNHO DO PEDIDO (vários itens de uma vez)
// ======================================================

function renderPedidoDraftList() {

  if (!pedidoDraftList) {
    return;
  }

  if (pedidoDraftItems.length === 0) {

    pedidoDraftList.innerHTML =
      "";
  ...
  pedidoDraftList.innerHTML =
    pedidoDraftItems
      .map(
        (item, index) => `
          <div class="pedido-draft-item">
            <span>
              ${escapeHtml(item.productName)} —
              <strong>${item.quantidade}</strong> pacotes
            </span>
            <button
              type="button"
              class="secondary pedido-draft-remove"
              data-index="${index}"
            >
              🗑️ Remover
            </button>
...
function resetPedidoProductFields() {
...
pedidoDraftList?.addEventListener(
  "click",
  event => {
    ...
    pedidoDraftItems.splice(
      index,
      1
    );

    renderPedidoDraftList();
  }
);


pedidoAddToListBtn?.addEventListener(
  "click",
  () => {
    ...
    if (!product) {
      ...
      showMessage(
        pedidoMessage,
        "Selecione o produto antes de adicionar à lista.",
        true
      );
    ...
    if (
      ...
      showMessage(
        pedidoMessage,
        "Informe uma quantidade necessária válida.",
        true
      );
    ...
    resetPedidoProductFields();

    showMessage(
      pedidoMessage,
      "Item adicionado à lista. Selecione outro produto ou clique em \"Criar pedido(s)\"."
    );
  }
);


createPedidoForm?.addEventListener(
  "submit",
  async event => {
    ...
    // Se a pessoa preencheu um produto mas esqueceu de clicar em
    // "Adicionar à lista", tratamos como mais um item da lista —
    // assim quem só quer criar 1 pedido não precisa clicar 2x.
    ...
    if (
      currentProduct &&
      Number.isInteger(currentQuantidade) &&
      currentQuantidade > 0
    ) {
    ...
    if (itemsParaCriar.length === 0) {
      ...
      showMessage(
        pedidoMessage,
        "Adicione pelo menos um produto com quantidade antes de criar.",
        true
      );
    ...
    const dataSaida =
      pedidoDataSaida.value ||
      null;

    const observacao =
      pedidoObservacao.value.trim() ||
      null;

    showMessage(
      pedidoMessage,
      `Criando ${itemsParaCriar.length} pedido(s)...`
    );
    ...
    if (erros.length > 0) {
      ...
      showMessage(
        pedidoMessage,
        `${itemsParaCriar.length - erros.length} de ${itemsParaCriar.length} pedido(s) criado(s). ` +
        `Erro em ${erros.length}: ${erros[0].error.message}`,
        true
      );
    ...
    showMessage(
      pedidoMessage,
      itemsParaCriar.length > 1
        ? `${itemsParaCriar.length} pedidos criados com sucesso!`
...
function formatHistoryProduct(item) {
  ...
  if (isRecheado) {
    ...
    produto =
      item.sabor
        ? `Pão de Queijo Recheado - ${item.sabor}`
        : "Pão de Queijo Recheado";

  } else {

    produto = `Pão de Queijo ${tipoLabel}`;
          ...
          if (
            ...
            const cargoAtual =
              isAdminRole(user.cargo)
                ? "admin"
                : isExpedicaoRole(user.cargo)
                  ? "expedicao"
                  : "funcionario";

            const roleSelect = `
              <select
                class="change-role-select"
              ...
              >
                <option
                  value="funcionario"
                  ${cargoAtual === "funcionario" ? "selected" : ""}
                >
                  👷 Funcionário
                </option>
...
async function changeUserRole(
  ...
  // O banco espera "administrador", "funcionario" ou "expedicao".
  ...
  async event => {
    ...
    if (isRevenda) {
      ...
      if (unidades > 0) {
        ...
        const result =
          await supabaseClient.rpc(
            "registrar_deposito_unidades_com_log",
            {
              p_produto_id:
                product.id,
    ...
    if (!isRevenda) {
      ...
      const pedidoAutoResult =
        await supabaseClient.rpc(
          "registrar_progresso_pedido_automatico",
          {
            p_produto_id:
              product.id,
      ...
      if (pedidoAutoResult.error) {

        // Não bloqueia a produção (já foi registrada com sucesso),
        // só avisa no console pra dar pra investigar.
        console.error(
          "Não consegui atualizar o progresso do pedido automaticamente:",
          pedidoAutoResult.error
        );
      }
    }
...
exitForm.addEventListener(
  ...
  async event => {
    ...
    if (unidades > 0) {
      ...
      const result =
        await supabaseClient.rpc(
          "registrar_retirada_unidades_com_log",
          {
            p_produto_id:
              product.id,
